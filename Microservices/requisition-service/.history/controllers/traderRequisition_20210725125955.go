...
func CreateRequisition(c *gin.Context) {
	...
	var input models.CreateRequisitionInput
	if err := c.ShouldBindJSON(&input); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	//create requisition
	requisition := models.Requisition{ProductType: input.ProductType, Quantity: input.Quantity, DeliveryLocation: input.DeliveryLocation, ExpectedDeliveryDate: input.ExpectedDeliveryDate, SpecialInstructions: input.SpecialInstructions, Repeats: input.Repeats, RepeatDate: input.RepeatDate, CreatorId: input.CreatorId}
